...
class Column {
 public:
  ...
  Column(const std::string& name, const DataType type,
    ...
    // if (type == DataType::INT) { 强行当long处理，ljz修改
    if (true) {
      // Convert string to long.
      data_.reserve(str_data.size());
      for (const std::string& str : str_data) data_.push_back(std::stol(str));
  ...
  static std::unique_ptr<Table> FromCsv(
    ...
    for (csv::CSVRow& row : reader) {
      for (size_t i = 0; i < column_names.size(); ++i) {
        std::string _tmp = row[column_infos[i].index].get<std::string>();
        csv_data[i].push_back(_tmp);
